...
from scripts.reports import maintenance_command_journal, maintenance_refresh_bundle, maintenance_resume_brief, maintenance_reports_index, maintenance_snapshot_report
...
def build_index(version: str, generated_at: datetime, included_reports: list[Path]) -> str:
    ...
    lines.append("- usar `python scripts/reports/maintenance_reports_index.py` quando quiser localizar rapidamente os artefatos mais recentes em `reports/`")
    ...
    lines.append("- Para localizar rapidamente os artefatos mais recentes já gerados em `reports/`, execute `python scripts/reports/maintenance_reports_index.py`.")
