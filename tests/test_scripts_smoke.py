...
from scripts.quality import canonical_paths_reference_guard, compatibility_contract_guard, operational_checks_contract_guard, release_metadata_guard, script_cli_contract_guard
...
class ScriptsSmokeTestCase(unittest.TestCase):
    ...
    def test_extractors_identificam_versoes_basicas(self) -> None:
        footer = 'versao: str = "v77"'
        changelog = '## v77 — 2026-04-10\n\nResumo:'
        baseline = 'A baseline oficial vigente desta base é **v77**.'

        self.assertEqual(release_metadata_guard.extract_footer_version(footer), 'v77')
        self.assertEqual(release_metadata_guard.extract_changelog_version(changelog), 'v77')
        self.assertEqual(release_metadata_guard.extract_baseline_version(baseline), 'v77')
    ...
    def test_release_health_report_build_report_tem_titulos(self) -> None:
        ...
        self.assertIn('RELEASE_HEALTH_REPORT — v77', report)
