...
from scripts.quality import canonical_paths_reference_guard, compatibility_contract_guard, operational_checks_contract_guard, release_metadata_guard
...
class ScriptsSmokeTestCase(unittest.TestCase):
    ...
    def test_extractors_identificam_versoes_basicas(self) -> None:
        footer = 'versao: str = "v76"'
        changelog = '## v76 — 2026-04-10\n\nResumo:'
        baseline = 'A baseline oficial vigente desta base é **v76**.'

        self.assertEqual(release_metadata_guard.extract_footer_version(footer), 'v76')
        self.assertEqual(release_metadata_guard.extract_changelog_version(changelog), 'v76')
        self.assertEqual(release_metadata_guard.extract_baseline_version(baseline), 'v76')
    ...
    def test_release_health_report_build_report_tem_titulos(self) -> None:
        ...
        self.assertIn('RELEASE_HEALTH_REPORT — v76', report)
