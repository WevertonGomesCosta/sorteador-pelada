...
from scripts.quality import canonical_paths_reference_guard, compatibility_contract_guard, documentation_commands_examples_guard, operational_checks_contract_guard, quality_runtime_budget_guard, release_artifacts_hygiene_guard, release_manifest_guard, release_metadata_guard, runtime_dependencies_contract_guard, script_cli_contract_guard
...
class ScriptsSmokeTestCase(unittest.TestCase):
    ...
    def test_extractors_identificam_versoes_basicas(self) -> None:
        footer = 'versao: str = "v82"'
        changelog = '## v82 — 2026-04-10\n\nResumo:'
        baseline = 'A baseline oficial vigente desta base é **v82**.'

        self.assertEqual(release_metadata_guard.extract_footer_version(footer), 'v82')
        self.assertEqual(release_metadata_guard.extract_changelog_version(changelog), 'v82')
        self.assertEqual(release_metadata_guard.extract_baseline_version(baseline), 'v82')
    ...
    def test_release_health_report_build_report_tem_titulos(self) -> None:
        ...
        self.assertIn('RELEASE_HEALTH_REPORT — v82', report)
